...
class PromptAutocompleteController {
  ...
  onKeyDown(event) {
    ...
    if (event.key === "ArrowUp") {
      event.preventDefault();
      const nextIndex = this.activeIndex <= 0 ? this.items.length - 1 : this.activeIndex - 1;
...
function isMarkedTargetInputElement(inputEl) {
  return isTextInputElement(inputEl) && inputEl.getAttribute(TARGET_INPUT_ATTR) === "1";
}
...
function isLikelyTargetInput(inputEl) {
  ...
  if (dataWidgetName === TARGET_WIDGET_NAME && (dataTestId === "dom-widget-textarea" || tagName === "textarea")) {
    return true;
  }

  // Very aggressive check for V2 prompt textareas
  if (tagName === "textarea" && (className.includes("comfy-") || className.includes("vue-") || className.includes("prompt") || placeholder.includes("prompt"))) {
    return true;
  }

  if (className.includes("comfy-multiline-input") || className.includes("comfy-textarea") || className.includes("comfy-input")) {
    return true;
  }

  const inputName = (inputEl.name || "").toLowerCase();
  if (inputName === TARGET_WIDGET_NAME && (placeholder.includes("autocomplete") || placeholder.includes("prompt"))) {
    return true;
  }
...
function getPromptInputElement(node) {
  ...
  const namedWidget = widgets.find(
    (candidate) => String(candidate?.name || "").toLowerCase() === TARGET_WIDGET_NAME
  );
  ...
  return textCandidates.find((inputEl) => isLikelyTargetInput(inputEl)) || null;
}
...
function findNodeIdFromElement(el) {
  ...
  while (current) {
    ...
    const attrId = typeof current.getAttribute === "function" ? current.getAttribute("data-node-id") : null;
    if (attrId) {
      return attrId;
    }
...
function installGlobalHooks() {
  ...
  document.addEventListener("keyup", (event) => {
    const target = getInputFromComposedEvent(event);
    if (target && isTextInputElement(target) && !controllerMap.has(target)) {
...
function patchComfyStringWidget() {
  ...
  ComfyWidgets.STRING = function patchedStringFactory(node, inputName, inputData) {
    ...
    try {
      ...
      console.error(`[${EXTENSION_NAME}] failed to attach via ComfyWidgets.STRING`, error);
    }
...
function patchComfyTextareaWidget() {
  ...
  ComfyWidgets.TEXTAREA = function patchedTextareaFactory(node, inputName, inputData) {
    ...
    try {
      ...
      console.error(`[${EXTENSION_NAME}] failed to attach via ComfyWidgets.TEXTAREA`, error);
    }
...
function patchNodeWidgetFactories() {
  ...
  if (typeof originalAddWidget === "function") {
    nodeProto.addWidget = function patchedAddWidget(type, name) {
      ...
      try {
        ...
        console.error(`[${EXTENSION_NAME}] failed to attach via LGraphNode.addWidget`, error);
      }
  ...
  if (typeof originalAddDOMWidget === "function") {
    nodeProto.addDOMWidget = function patchedAddDOMWidget(name, type, element) {
      ...
      try {
        ...
        console.error(`[${EXTENSION_NAME}] failed to attach via LGraphNode.addDOMWidget`, error);
      }
